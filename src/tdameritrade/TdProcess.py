...
def get_realvol(cod, td):
    ...
    parm = dict(apikey=td_api_key, periodType='month', frequencyType='daily')
    jsondata = call_tda(urlPrice, parm)
    # jsondata = get_hvol_from_td(urlPrice, parm, retries=3, delay=3.0)
    knt = len(jsondata)
    if knt < 1:
    ...
    try:
        ...
        if len(cnd) < 1:
            print("get_realvol() with no cnd data for ", cod)
        # print_json(cnd)
        ...
        print("get_realvol() with exception for ", cod)
        rv = float(0.0)

    return rv


def get_hvol_from_td(urlstr, parm, retries, delay):
    contin = True
    knt = 0
    while contin:
        data = call_tda(urlstr, parm)
        knt = knt + 1
        if data['empty']:
            print("  data empty")
            if knt >= retries:
...
def get_avg_iv2(jsondata, underlying, debug):
    ivtot = float(0.0)
    knt = int(0)
    for expdate in jsondata.keys():
        for strike in jsondata[expdate].keys():
            for option in jsondata[expdate][strike]:
                opt = OptionData(option, underlying)
                if opt.valid:
                    if 19 < opt.daysToExpiration < 183:
                        if opt.oi > 0:
                            if opt.volatility > 0.0:
                                if abs(opt.ul_pct) < 30.0:
                                    ivtot += (opt.volatility * float(opt.oi))
                                    knt += opt.oi
                                    if debug:
                                        print(option['symbol'], opt.volatility, ivtot, knt, opt.daysToExpiration)
    aiv = float(0.0)
